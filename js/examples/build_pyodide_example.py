...
def main(output: Path, table_wheel: Path, dependency_dir: Path) -> None:
    spaday_wheel = one_wheel(dependency_dir, "spaday")
    wheels = {
        "table": copy_wheel(table_wheel, output),
        "spaday": copy_wheel(spaday_wheel, output),
    }
    extract_assets(table_wheel, "spaday_regular_table", output / "components" / "regular-table")
