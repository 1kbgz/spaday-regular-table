...
lint-py:  ## run python linter with ruff
	python -m ruff check spaday_regular_table js/examples/build_pyodide_example.py
	python -m ruff format --check spaday_regular_table js/examples/build_pyodide_example.py
...
fix-py:  ## fix python formatting with ruff
	python -m ruff check --fix spaday_regular_table
	python -m ruff format spaday_regular_table
...
test-py:  ## run python tests
	python -m pytest -v spaday_regular_table/tests

# alias
tests-py: test-py

coverage-py:  ## run python tests and collect test coverage
	python -m pytest -v spaday_regular_table/tests --cov=spaday_regular_table --cov-report term-missing --cov-report xml
...
pyodide-example: build  ## build the standalone Pyodide example into dist/lite
	test -n "$(firstword $(wildcard dist/spaday_regular_table-*.whl))"
	...
	python js/examples/build_pyodide_example.py dist/lite "$(firstword $(wildcard dist/spaday_regular_table-*.whl))" dist/pyodide-deps
...
test-pyodide-example: pyodide-example  ## run the standalone Pyodide example in Chromium
	...
	cd js; SPADAY_REGULAR_TABLE_PYODIDE_ONLY=1 pnpm exec playwright test tests/pyodide.spec.js
